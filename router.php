...
// 2. Doctor Profile Rule
if (preg_match('/^\/doctor\/([a-zA-Z0-9-]+)$/', $path, $matches)) {
    $_GET['id'] = $matches[1];
    require 'doctor-details.php';
    return true;
}
...
// 3. Marathon Rule
if (preg_match('/^\/marathons\/([a-zA-Z0-9-]+)$/', $path, $matches)) {
    $_GET['slug'] = $matches[1];
    require 'marathon-details.php';
    return true;
}

// 4. Challenge Rule
if (preg_match('/^\/challenges\/([a-zA-Z0-9-]+)$/', $path, $matches)) {
    $_GET['slug'] = $matches[1];
    require 'challenge-details.php';
    return true;
}
